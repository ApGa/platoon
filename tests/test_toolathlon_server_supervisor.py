...
MULTIENV_SERVER = REPO_ROOT / "slurm-scripts" / "openreward-multienv-server.sh"
...
def test_toolathlon_supervisor_has_stable_port_bounded_restart_semantics():
    ...
    assert "WORKER_PID_ISOLATION=${OPENREWARD_WORKER_PID_ISOLATION:-1}" in script
    for namespace_arg in (
        "--map-current-user",
        "--pid",
        "--fork",
        "--kill-child=SIGKILL",
        ...
        assert namespace_arg in script
    assert '"${worker_namespace_args[@]}" env -u OPENREWARD_PORT' in script
...
def test_toolathlon_launcher_mounts_supervisor_without_bad_exit_cascade():
    ...
    assert "toolathlon_backend_healthy" in launcher
    assert "--noproxy '*'" in launcher
    assert '[[ "${status}" =~ ^[1-4][0-9][0-9]$ ]]' in launcher
    ...
    assert "--no-container-mount-home" in launcher
...
def test_multienv_config_uses_proven_straggler_tail_policy():
    ...
    assert "group_size: 8" in contents
    ...
    assert "request_timeout: 12600" in contents
    assert "session_timeout_seconds: 7200" in contents
    assert "freq_steps: 1" in contents
...
def test_multienv_servers_preserve_enroot_import_caps_and_use_restart_supervisor():
    subprocess.run(["bash", "-n", str(MULTIENV_SERVER)], check=True)
    server = MULTIENV_SERVER.read_text()

    assert MULTIENV_SERVER.stat().st_mode & 0o111
    assert "enroot-aufs2ovlfs" in server
    assert "nested user namespace" in server
    assert "PID_NAMESPACE_ARGS" not in server
    assert '    "$@" &' in server
    assert "supervise_server" in server
    assert 'SERVER_COMMAND=("${PYTHON}" -m openreward_env.server)' in server
    assert 'SERVER_COMMAND=("${PYTHON}" server.py)' in server
    assert 'exec "${PYTHON}"' not in server
    assert "server exited unexpectedly" in server
    assert "server exhausted its restart budget" in server
