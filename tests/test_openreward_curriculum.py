...
def test_nonrecursive_toolathlon_swe_curriculum_composes_from_fresh_trial():
    ...
    assert config.trial_name == "ta-swe16-ptc-ta20-v1-trial0"
    assert config.stats_logger.wandb.group == "ta-swe16-ptc-ta20-v1"
...
def test_recursive_toolathlon_swe_curriculum_preserves_efficiency_settings():
    ...
    _assert_two_phase_catalog(config)
    assert config.trial_name == "ta-swe32-rec-ta20-v1-trial0"
    assert config.stats_logger.wandb.group == "ta-swe32-rec-ta20-v1"
