...
def patch_proxy_chat_template_kwargs():
    ...
    def patched_build_app(client=None, session_cache=None):
        ...
        app.router.routes = [r for r in app.router.routes if not (hasattr(r, 'path') and r.path == "/v1/{session_id}/chat/completions")]
        ...
        async def chat_completions(
            ...
            dropped_non_default_args = [
                ...
                and k not in SILENTLY_DROPPED_ARGS
            ]
    ...
    async def patched_create(
        ...
        **kwargs: Any,
        ...
        if not is_omitted(max_tokens) and max_tokens is not None:
            calculated = max_tokens - len(prompt_token_ids)
            if calculated <= 0:
                raise RuntimeError(
