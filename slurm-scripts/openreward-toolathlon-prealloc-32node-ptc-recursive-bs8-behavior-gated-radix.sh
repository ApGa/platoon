#!/bin/bash
#SBATCH --job-name=openreward-ta32-rec-behavior-gated-radix
...
#SBATCH --output=/lustre/fsw/portfolios/nvr/projects/nvr_lacr_llm/users/apurvag/logs/openreward-ta32-rec-behavior-gated-radix-%j.out
#SBATCH --error=/lustre/fsw/portfolios/nvr/projects/nvr_lacr_llm/users/apurvag/logs/openreward-ta32-rec-behavior-gated-radix-%j.err
...
DEFAULT_CONFIG=${REPO_ROOT}/plugins/openreward/platoon/openreward/configs/areal/toolathlon_openhands_areal_prealloc_32node-cp-ptc-recursive-behavior-gated-radix-r3-fp32-lm-head-bs8.yaml
...
# Match the latest behavior-gated deadline policy: exclude the cold first update
# from adaptive history, retain the 30-minute floor, and add no fixed buffer.
...
# Keep this radix-cache ablation isolated from every existing recovery lineage.
export OPENREWARD_TRIAL_NAME=ta32-rec-behavior-gated-radix-v1-trial0
export OPENREWARD_SUBAGENT_DELEGATION_REWARD_COEFFICIENT=0.0
export OPENREWARD_WANDB_MODE=online
export OPENREWARD_JOB_SCRIPT=${REPO_ROOT}/slurm-scripts/openreward-toolathlon-prealloc-32node-ptc-recursive-bs8-behavior-gated-radix.sh
