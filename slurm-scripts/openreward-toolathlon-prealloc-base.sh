...
ROLLOUT_IDLE_GUARD_READY_DIR=${OPENREWARD_JOB_STATE_DIR}/rollout-idle-guard-ready
...
# scheduler starts it only for the exact rollout role and pins it to that role's
# resolved nodes. The Python guard independently rejects configurations above a
# 5% duty cycle or a 2048 matrix dimension.
export PLATOON_AREAL_ROLLOUT_IDLE_GUARD=${PLATOON_AREAL_ROLLOUT_IDLE_GUARD:-1}
export ROLLOUT_IDLE_GUARD_INTERVAL_SECONDS=${ROLLOUT_IDLE_GUARD_INTERVAL_SECONDS:-30}
...
export ROLLOUT_IDLE_GUARD_BURST_SECONDS=${ROLLOUT_IDLE_GUARD_BURST_SECONDS:-1}
...
mkdir -p "${ROLLOUT_IDLE_GUARD_READY_DIR}"
rm -f "${ROLLOUT_IDLE_GUARD_READY_DIR}"/*.ready "${ROLLOUT_IDLE_GUARD_READY_DIR}"/.*.tmp
mkdir -p "${STOP_DIR}"
...
export PLATOON_AREAL_ROLLOUT_IDLE_GUARD_LOG_PREFIX="${USER_ROOT}/logs/rollout-idle-guard-${RUN_ID}-${JOB_INSTANCE_ID}"
